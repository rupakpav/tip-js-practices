...
**Изучены среды выполнения:** На практике исследованы различия между браузерным окружением (где доступен объект `document`) и серверной платформой Node.js (где `document` возвращает `undefined`).
...
**Закреплены управляющие конструкции:** Написана логика выполнения задач с помощью (`if-else if`).

**Применены навыки отладки:** На примере файла `debug.js` освоена пошаговая отладка кода (`Step over`) с использованием точек останова (`Breakpoints`) и панелей отслеживания переменных (`Scope`) в инструментах разработчика Firefox.
